...
<template>
	<div :class="['sunset-search-form-container',right?'pull-right':'']">
		<form :class="['sunset-search-form form-inline form-horizontal',right?'pull-right':'']" onsubmit="return false">
			<filter-field v-for="(field,index) in fields" :key="index" :options="field" :filter="filter" v-model="filter[field.name]" @search="fieldTriggerSearch" @ready="fieldReady"></filter-field>
...
export default {
    ...
    computed: {
        fields() {
            ...
            return ((this.options && this.options.fields) || []).filter(
                field => (field.premise ? field.premise(filter) : true)
            );
        },
        right() {
    ...
    methods: {
        ...
        fieldReady(fieldName) {
            if (
                this.waitReadyWidgetMap[fieldName] &&
                this.waitReadyWidgetCounter > 0
            ) {
                this.waitReadyWidgetCounter--;
                if (this.waitReadyWidgetCounter == 0) {
                    if (!this.lock) {
