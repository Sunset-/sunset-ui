				...
				<table :class="['table table-bordered table-striped',options.condensed?'table-condensed':'']">
					...
					<tbody>
						<tr v-for="(item,index) in list" :key="index">
							...
							<td v-for="col in showColumns" :style="col.style||{'text-align':col.align}" v-html=" item.__sunset_col_texts[col.__sunset_col_index]"></td>
							<td v-if="recordTools" class="sunset-table-record-tools text-center">
								<div>
									<div>
    ...
    computed: {
        ...
        showColumns() {
            return this.columns.filter(
                col => !!col.name && (col.premise ? col.premise() : true)
            );
        },
        //行操作栏
        recordTools() {
            ...
            if (recordTools) {
                if (Sunset.isArray(recordTools) && recordTools.length) {
                    ...
                    Sunset.isArray(recordTools.tools) &&
                    recordTools.tools.length
                ) {
                    result = recordTools;
                    if (recordTools.size == void 0) {
                        recordTools.size = Sunset.getConfig(
                            "TABLE_TOOLBAR_SIZE"
                        );
                    }
                }
        ...
        recordToolsWidth() {
            if (this.options.recordToolsWidth) {
                ...
                if (recordTools) {
                    var tools = Sunset.isArray(recordTools)
                            ? recordTools
                            : recordTools.tools,
                        space = recordTools.size == "small" ? 26 : 38;
                    tools.forEach(t => {
                        w +=
                            ((t.label && t.label.length) || 0) * 14 +
                            (t.icon ? 20 : 0) +
                            space;
                    });
                }
    ...
    methods: {
        ...
        refresh(pageNumber, slient, fromPage) {
            ...
            filter[currentPagePlace] =
                this.options.pageNumberStart === 0
                    ? pageNumber - 1
                    : pageNumber;
            ...
            if (!fromPage || !this.isLocalPage || !this.data) {
                ...
                var datasource =
                    this.datasource ||
                    (this.store &&
                        this.store[this.options.method || "list"].bind(
                            this.store
                        ));
                Promise.resolve(
                    (() => {
                        if (datasource) {
                            ...
                            return Sunset.isFunction(datasource)
                                ? datasource(filter)
                                : datasource;
                        } else {
                            throw new Error(
                                "table load data need datasource or store"
                            );
                        }
                    })()
                )
                    .then(res => {
                        ...
                        if (
                            list.length == 0 &&
                            count > 0 &&
                            filter[currentPagePlace] > pageStart
                        ) {
                            filter[currentPagePlace] =
                                filter[currentPagePlace] - 1;
                            return Sunset.isFunction(datasource)
                                ? datasource(filter)
                                : datasource;
                        } else {
        ...
        setData(res) {
            ...
            if (this.format && this.format["list"] == "") {
                ...
                list =
                    res &&
                    Sunset.getAttribute(res, this.format["list"] || "list", []);
            }
            list = list.map(record => {
                ...
                columns.forEach((col, index) => {
                    __sunset_col_texts[
                        index
                    ] = Sunset.Service.Common.tableColTranscode(
        ...
        refreshAfterRemove() {
            if (
                this.pageNumber > 1 &&
                this.count - 1 == (this.pageNumber - 1) * this.pageSize
            ) {
