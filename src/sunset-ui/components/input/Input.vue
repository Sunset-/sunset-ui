...
<template>
    <div :class="['sunset-input',options.className||'',!options.icon?'sunset-input-noicon':'']">
        <i-input @on-blur="blur" :type="type" v-model="inputValue" :maxlength="maxlength" :disabled="options.disabled" :clearable="options.clearable" :readonly="options.readonly" :placeholder="options.placeholder" :icon="options.icon" :size="options.size" :style="options.style">
            <!-- prepend -->
            <span v-if="prependText" slot="prepend" v-html="prependText"></span>
            <i-select v-if="prependSelect" v-model="prependValue" slot="prepend" :style="prependSelect.style">
                <i-option v-for="item in prependItems" :key="item.value" :value="item.value">{{ item.text }}</i-option>
            </i-select>
            <!-- append -->
            <span v-if="appendText" style="cursor:pointer;" @click="clickButton" slot="append" v-html="appendText"></span>
            <i-select v-if="appendSelect" v-model="appendValue" slot="append" :style="appendSelect.style">
                <i-option v-for="item in appendItems" :key="item.value" :value="item.value">{{ item.text }}</i-option>
            </i-select>
        </i-input>
    </div>
</template>
...
export default {
    ...
    computed: {
        type() {
            return (
                (this.options.type == "input" || this.options.type == "number"
                    ? "text"
        ...
        isNumber() {
            return (
                this.options.widget == "number" || this.options.type == "number"
            );
        },
        digits() {
        ...
        prependText() {
            return this.options.prepend && Sunset.isString(this.options.prepend)
                ? this.options.prepend
                : false;
        },
        prependSelect() {
            return this.options.prepend && Sunset.isObject(this.options.prepend)
                ? this.options.prepend
                : false;
        },
        prependSpliter() {
        ...
        appendText() {
            return this.options.append && Sunset.isString(this.options.append)
                ? this.options.append
                : false;
        },
        appendSelect() {
            return this.options.append && Sunset.isObject(this.options.append)
                ? this.options.append
                : false;
        },
        appendSpliter() {
                ...
                if (
                    ...
                    if (!isNaN(this.inputValue)) {
                        ...
                        if (
                            ...
                            +this.inputValue > +this.options.max
                        ) {
                            this.inputValue =
                                Sunset.Numbers.fixed(
                                    this.options.max,
