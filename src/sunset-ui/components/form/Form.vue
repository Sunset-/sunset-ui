...
<template>
	<form :class="['sunset-form form-horizontal',showWarning?'sunset-form-show-warning':'']" @submit.prevent="submit">
        <Row>
			<template v-for="field in renderFields" ref="fields">
				<span v-html="newline(field)"></span>
				<div v-if="field.group" :class="['ivu-col ivu-col-span-24',field.groupClassName]" :style="field.groupStyle">
				...
				<i-col :span="computedFieldClass(field)" :style="field.fieldStyle">
					<sunset-field ref="field" :options="field" :form-options="options" v-model="model[field.name]" :model="model" @ready="promiseWidgetReady" @change="fieldValueChange" @remove="removeField(field)"></sunset-field>
				</i-col>
			</template>
		</Row>
		<!-- tip -->
		...
		<!-- toolbar -->
		<div v-if="options.tools!==false&&options.toolbar!==false" style="text-align:center;">
    ...
    computed: {
        renderFields() {
            ...
            fields = fields.filter(
                field => (field.premise ? field.premise(model) : true)
            );
            //初始化
            if (record) {
                fields = fields.map(f => {
    ...
    methods: {
            ...
            fields.forEach(field => {
                if (field.name) {
                    this.$set(
                        this.model,
                        field.name,
                        this.defaultValueMap[field.name]
                    );
                    ...
                    if (defaulValue) {
                        ...
                        prall.push(
                            Promise.resolve().then(() => {
                                return Sunset.isFunction(defaulValue)
                                    ? defaulValue()
                                    : defaulValue;
                            })
                        );
            ...
            this.generateModel()
                .then(model => {
                    if (Sunset.isFunction(this.options.submit)) {
                        ...
                        this.options.store[this.options.method || "save"](model)
                            .then(res => {
                                Sunset.tip(
                                    this.options.successTip || "保存成功",
                                    "success"
                                );
                                this.$emit("signal", "SAVED", res, model);
                            })
        ...
        newline(field) {
            return field.newline
                ? `</div><div style="z-index:-1;" class="ivu-row">`
                : "";
        },
        rebuild(model) {
