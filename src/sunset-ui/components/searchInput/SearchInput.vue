...
<template>
    <div :class="['sunset-searchinput',all?'have-all':'',size,!multi?'no-multi':'']" @click="focus">
        ...
        <input @input="inputSearch" @keydown.stop="toSelect" @blur="blur" :placeholder="placeholder" :maxlength="maxlength" v-model="inputValue" type="text" />
        <div :style="popStyle" v-show="searchResults.length" class="sunset-searchinput-searchresults">
            <div :class="['sunset-searchinput-searchresult',activeSelectItem==sr?'active':'',isSelected(sr)?'selected':'']" v-for="(sr,index) in searchResults" @click.stop="select(sr,true,true)" :key="index" v-html="template(sr)">
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...
        delay() {
            return Sunset.isNumber(this.options && this.options.delay)
                ? this.options.delay
                : 100;
        },
        search() {
            return Sunset.isFunction(this.options && this.options.search)
                ? this.options.search
                : null;
        },
        check() {
            return Sunset.isFunction(this.options && this.options.check)
                ? this.options.check
                : null;
        },
        multi() {
            return (
                this.options && (this.options.multi || this.options.multiple)
            );
        },
        spliter() {
        ...
        inputable() {
            return !(
                this.options.input === false || this.options.inputable === false
            );
        },
        template() {
            return Sunset.isFunction(this.options && this.options.template)
        ...
        format() {
            return Sunset.isFunction(this.options && this.options.format)
                ? this.options.format
                : function(v) {
                      return v;
                  };
        ...
        formatTemplate() {
            return Sunset.isFunction(
                this.options && this.options.formatTemplate
            )
    ...
    methods: {
        toSelect(ev) {
            switch (ev.keyCode) {
                ...
                case 13:
                    ...
                    if (
                        ...
                        ~this.searchResults.indexOf(this.activeSelectItem)
                    ) {
            ...
            setTimeout(() => {
                ...
                var $activer = $(
                    ".sunset-searchinput-searchresult.active",
                    this.$el
                );
                ...
                if ($activer && $activer.length) {
                    var offsetParentTop =
                            $activer.offset().top -
                            $activer.offsetParent().offset().top,
        ...
        activeSearchResultItem(which) {
            var list = this.searchResults,
                ...
                length = list.length;
            if (current == null) {
                current = which > 0 ? list[0] : list[length - 1];
            } else {
        ...
        select(v, fromSearchResult, fromClick) {
            ...
            if (fromSearchResult) {
                ...
                Sunset.isFunction(this.options && this.options.onSelected) &&
                    this.options.onSelected.call(null, v);
                text = this.format(v);
            }
            if (this.multi) {
