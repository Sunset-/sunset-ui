<template>
	<div class="sunset-field">
		<i-input v-model="text" @click.native="select(true)" :placeholder="options.placeholder" @on-blur="blur" :readonly="!inputable">
			<i-button @click.stop="select()" slot="append">选择</i-button>
		</i-input>
		<sunset-table-modal v-if="inited" @submit="tableSelected" ref="tablemodal" :options="options.modalOptions"></sunset-table-modal>
	</div>
</template>
<script>
export default {
        ...
        watchedValue(v) {
            if (!this.lock) {
                ...
                if (v && this.options.transcode) {
                    Promise.resolve(
                        this.options.transcode(v.split(this.spliter))
                    ).then(text => {
